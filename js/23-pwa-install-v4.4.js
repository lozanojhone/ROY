(function () {
  ...
  const INSTALLED_KEY = 'roy_pwa_installed_v2';
  const DAY = 86400000;
  ...
  function markInstalled(source) {
    try {
      ...
      localStorage.setItem(INSTALLED_KEY + '_source', source || 'detected');
      localStorage.setItem(INSTALLED_KEY + '_at', String(Date.now()));
      localStorage.removeItem(DISMISS_KEY);
    } catch (_) {}
  }

  function clearInstalledMarker() {
    try {
      localStorage.removeItem(INSTALLED_KEY);
      localStorage.removeItem(INSTALLED_KEY + '_source');
      localStorage.removeItem(INSTALLED_KEY + '_at');
    } catch (_) {}
  }
  ...
  function updateUI() {
    ...
    installed = isStandalone() || hasInstalledMarker();
  ...
  function renderInstructions() {
    ...
    if (isIOS()) {
      ...
      setStatus(
        'Después de agregar ROY a tu pantalla de inicio, vuelve aquí y pulsa “YA LA AGREGUÉ”. Safari no permite comprobar automáticamente una instalación hecha desde su menú.'
      );
  ...
  function showBanner(force) {
    ...
    if (banner) {
      setTimeout(function () {
        if (!installed && !isStandalone() && !hasInstalledMarker()) {
  ...
  window.addEventListener(
    ...
    function (event) {
      ...
      // en este perfil. Esto limpia una marca antigua, por ejemplo tras desinstalarla.
      if (!isStandalone() && hasInstalledMarker()) {
