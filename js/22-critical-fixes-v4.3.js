...
const oldShow=window.showAdminView;
window.showAdminView=function(name,btn){const r=oldShow.apply(this,arguments);if(name==='users'&&!usersViewLoading){usersViewLoading=true;try{window.renderUsers?.()}catch(e){console.error(e)}setTimeout(async()=>{try{await window.syncUsersFromFirestore?.(false);await window.loadUserAudit?.(false)}catch(e){console.warn('Usuarios:',e)}finally{usersViewLoading=false}},80)}return r};
