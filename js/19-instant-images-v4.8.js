...
(()=>{
  ...
  function paint(product,src){
    ...
    product.imageThumb=src;
    if(!valid(product.image))product.image=src;
  ...
  async function resolve(product,persist=false){
    ...
    if(valid(product.imageThumb)||valid(product.image))return paint(product,product.imageThumb||product.image);
  ...
  async function migrateAll(){
    ...
    const missing=products.filter(p=>!valid(p.imageThumb)&&!valid(p.image)&&p.imageAssetId);
    if(!missing.length){
      products.forEach(p=>paint(p,p.imageThumb||p.image));
      return;
    }
