...
(()=>{
  const FALLBACK='assets/logo-roy-verde-blanco.png';
  ...
  const safeImages=(p,color='')=>{
    ...
    const general=unique([p.imageThumb,p.image,...(Array.isArray(p.gallery)?p.gallery:[])]);
    if(!color)return general.length?general:[FALLBACK];
    const own=unique(normalizeColorImages(p.colorImages||{})[color]||[]);
    return own.length?own:(general.length?general:[FALLBACK]);
  };
  ...
  function setMainImage(src,index=0){
    ...
    if(!img)return;
    const previous=img.currentSrc||img.src||FALLBACK;
    img.classList.add('roy-image-switching');
    loadImage(src).then(valid=>{
      img.onload=()=>img.classList.remove('roy-image-switching');
      img.onerror=()=>{img.onerror=null;img.src=previous||FALLBACK;img.classList.remove('roy-image-switching')};
      img.src=valid;
      quickImageIndex=index;
    }).catch(()=>{
      if(!img.src)img.src=previous||FALLBACK;
      img.classList.remove('roy-image-switching');
    });
  }

  function thumbMarkup(src,i,context){
    return `<button type="button" class="quick-thumb ${i===quickImageIndex?'active':''}" onclick="chooseQuickImage(${i},this)" aria-label="${esc(context)} vista ${i+1}"><img loading="${i<3?'eager':'lazy'}" decoding="async" src="${esc(src)}" alt="${esc(context)} · vista ${i+1}" onerror="this.onerror=null;this.src='${FALLBACK}'"></button>`;
  }
  ...
  window.renderQuickGallery=function(p,color=''){
    ...
    setMainImage(quickImageList[0]||FALLBACK,0);
  ...
  window.chooseQuickImage=function(index,el){
    const src=quickImageList[index]||quickImageList[0]||FALLBACK;
  ...
  window.chooseQuick=async function(type,index,el){
    ...
    if(before.length&&before[0]!==FALLBACK)renderQuickGallery(p,quickColor);
    try{
  ...
  window.openQuick=async function(id){
    ...
    // Render inicial inmediato: nunca muestra un rectangulo vacio.
    renderQuickGallery(p,'');
