...
(function(){
 ...
 const rolePermissions={
  ...
  'Contenido':['dashboard','catalog','gallery','upcoming','sale','promotions','settings'],
  'Atención al cliente':['dashboard','orders','clients','messages','reviews']
 };
 function canView(view){const p=window._firebasePermissions||[];const permission=view==='upcoming'?'upcoming':view==='sale-content'?'sale':view;return p.includes('*')||p.includes(permission)}
 ...
 function applyRoleMenu(){const buttons=QA('#adminMenu button[data-view]');buttons.forEach(b=>b.style.display=canView(b.dataset.view)?'':'none');const active=Q('.admin-view.active');const activeView=active?.id?.replace(/^admin-/,'')||'';if(activeView&&!canView(activeView)){const first=buttons.find(b=>canView(b.dataset.view));if(first)showAdminView(first.dataset.view,first)}const label=Q('#syncLabel');if(label&&window._firebaseRole)label.textContent='Firebase activo · '+window._firebaseRole}
