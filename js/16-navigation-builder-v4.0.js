...
(function(){
 ...
 const canonicalCoreTarget=item=>{
  const id=String(item?.id||'').trim().toLowerCase();
  const label=String(item?.label||'').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
  const target=String(item?.target||'').trim();
  // Las páginas principales de ROY conservan siempre su destino real aunque exista
  // una configuración antigua guardada en Firestore con un target incorrecto.
  if(id==='menu_sale'||label==='sale'||label==='sale / ofertas')return 'sale';
  if(id==='menu_proximamente'||label==='proximamente')return 'proximamente';
  if(id==='menu_inicio'||label==='inicio')return 'inicio';
  return target||'tienda';
 };
 const menuItems=()=>{
  const raw=Array.isArray(state?.settings?.navigation)?state.settings.navigation:DEFAULT_MENU;
  return raw.map((x,i)=>{
   const item={id:x.id||('menu_'+Date.now()+'_'+i),label:x.label||'Sección',type:x.type||'page',target:x.target||'tienda',active:x.active!==false,highlight:!!x.highlight,order:Number(x.order??((i+1)*10)),icon:x.icon||'',description:x.description||'',banner:x.banner||'',slug:x.slug||''};
   const coreTarget=canonicalCoreTarget(item);
   if(coreTarget==='sale'||coreTarget==='proximamente'||coreTarget==='inicio'){item.type='page';item.target=coreTarget;}
   return item;
