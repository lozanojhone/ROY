...
(function(){
 ...
 async function loadMainWithRetry(p,attempt=0){
  ...
  if(p._mainFull&&validImage(p.image))return applyMainImage(p,p.image);
 ...
 window.saveProduct=async function(){ensure();const btn=Q('#productModal button[onclick="saveProduct()"]');if(btn?.disabled)return;const id=Q('#productId').value.trim()||uid('prd'),old=state.products.find(x=>x.id===id)||{},name=Q('#pName').value.trim(),category=Q('#pCategory').value.trim(),price=Number(Q('#pPrice').value),oldPrice=Number(Q('#pOldPrice').value||0),stock=Number(Q('#pStock').value);
  ...
  btn.disabled=true;const prev=btn.innerHTML;btn.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Guardando';try{
   st('Comprimiendo y guardando imagen principal...','loading');let imageAssetId=V.mainAssetId,mainThumb=V.mainThumbExisting||old.imageThumb||'',mainOriginal=(old._mainFull&&old.image)||'';if(V.main){const payload=await compress(V.main);imageAssetId=await putAsset(id,'main',payload,'',0,imageAssetId);mainThumb=payload.thumbData||payload.data||mainThumb;mainOriginal=payload.data||mainOriginal}else if(imageAssetId){const existingMain=await getAsset(imageAssetId);mainOriginal=existingMain?.data||mainOriginal;mainThumb=existingMain?.thumbData||mainThumb}
   ...
   const sku=old.sku||Q('#v20Sku').value||nextProductId(),sizes=parseList(Q('#pSizes').value);const p={...old,id,sku,name,category,brand:Q('#pBrand').value.trim()||'ROY',price,oldPrice,stock,tags:V.tags,colors:Object.keys(V.colors),sizes:sizes.length?sizes:['Única'],description:Q('#pDescription').value.trim(),imageAssetId,galleryAssetIds,colorImageAssetIds,image:mainOriginal||mainThumb,imageThumb:mainThumb,imageOriginalVersion:52,gallery:[],colorImages:{},material:Q('#pMaterial')?.value.trim()||'',composition:Q('#pComposition')?.value.trim()||'',fit:Q('#pFit')?.value.trim()||'',rise:Q('#pRise')?.value.trim()||'',gender:Q('#pGender')?.value||'Unisex',model:Q('#pModel')?.value.trim()||'',createdAt:old.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};
