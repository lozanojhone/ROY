...
function mediaAdminCard(m){const section=mediaSection(m),status=mediaAvailabilityMeta(m.availability,section),commerce=section!=='gallery';return `<article class="media-card"><div class="media-preview">${m.type==='image'?`<img src="${esc(m.url)}">`:videoMarkup(m.url,m.title)}</div><div class="media-body"><h4>${esc(m.title)}</h4>${commerce?`<div class="campaign-admin-commerce">${Number(m.price||0)>0?`<b>${money(m.price)}</b>`:'<span>Sin precio</span>'}<span class="campaign-status ${status.cls}">${esc(status.label)}</span></div>`:''}<p class="muted" style="font-size:10px;min-height:30px">${esc(m.description||'')}</p><div class="action-row" style="margin-top:10px"><button class="btn btn-secondary btn-sm" onclick="editMedia('${m.id}')">Editar</button><button class="btn btn-secondary btn-sm" onclick="toggleMedia('${m.id}')">${m.active===false?'Publicar':'Ocultar'}</button><button class="btn btn-danger btn-sm" onclick="deleteMedia('${m.id}')">Eliminar</button></div></div></article>`}
function renderMediaAdminSection(section){const meta=mediaSectionMeta(section),box=$('#'+meta.grid);if(!box)return;const q=($('#'+meta.search)?.value||'').toLowerCase(),type=$('#'+meta.type)?.value||'';const list=state.media.filter(x=>mediaSection(x)===section&&(!q||[x.title,x.description,x.type].join(' ').toLowerCase().includes(q))&&(!type||x.type===type));box.innerHTML=list.length?list.map(mediaAdminCard).join(''):`<div class="empty" style="grid-column:1/-1">No hay contenido en ${meta.label}.</div>`}
...
function openMediaForm(section='gallery'){['mediaId','mediaTitle','mediaUrl','mediaDescription','mediaPrice'].forEach(id=>$('#'+id).value='');$('#mediaType').value='image';$('#mediaAvailability').value=section==='upcoming'?'coming-soon':'available';$('#mediaActive').checked=true;resetMediaUpload();prepareMediaModal(section,'Nuevo <span>contenido</span>')}
function openSectionMediaForm(section){openMediaForm(section)}
function editMedia(id){const m=state.media.find(x=>x.id===id);if(!m)return;const section=mediaSection(m);$('#mediaId').value=m.id;$('#mediaType').value=m.type;$('#mediaTitle').value=m.title;$('#mediaUrl').value=m.url;$('#mediaDescription').value=m.description||'';$('#mediaPrice').value=Number(m.price||0)>0?Number(m.price):'';$('#mediaAvailability').value=mediaAvailabilityMeta(m.availability,section).value;$('#mediaActive').checked=m.active!==false;resetMediaUpload();prepareMediaModal(section,'Editar <span>contenido</span>')}
...
async function toggleMedia(id){const m=state.media.find(x=>x.id===id);if(!m)return;m.active=m.active===false;await putRecord('media',m);renderGallery();renderAdminMedia();renderAdminSectionMedia('upcoming');renderAdminSectionMedia('sale')}
async function deleteMedia(id){if(!confirm('¿Eliminar este contenido?'))return;await removeRecord('media',id);state.media=state.media.filter(x=>x.id!==id);saveLocal('media',state.media);renderGallery();renderAdminMedia();renderAdminSectionMedia('upcoming');renderAdminSectionMedia('sale')}
