...
function renderCampaignSections(){[['upcoming','upcomingCampaignImage','upcomingEyebrow','upcomingTitle','upcomingDescription'],['sale','saleCampaignImage','saleEyebrow','saleTitle','saleDescription']].forEach(([kind,imageId,eyeId,titleId,descId])=>{const c=campaignSettings(kind),img=$('#'+imageId),eye=$('#'+eyeId),title=$('#'+titleId),desc=$('#'+descId),container=img?.closest('.campaign-public');if(img){if(c.image){img.onload=()=>{img.hidden=false;container?.classList.add('has-image')};img.onerror=()=>{img.hidden=true;container?.classList.remove('has-image')};img.src=settingsImageUrl(c.image);img.hidden=false;container?.classList.add('has-image')}else{img.removeAttribute('src');img.hidden=true;container?.classList.remove('has-image')}}if(eye)eye.textContent=c.eyebrow;if(title)title.innerHTML=campaignTitleHtml(c.title,c.highlight);if(desc)desc.textContent=c.description})}
...
async function saveSettings(){
 ...
 try{
  ...
  saveLocal('settings',[row]);localStorage.setItem('roy_brand_cache',JSON.stringify({storeName:row.storeName,logo:row.logo,primaryColor:row.primaryColor,secondaryColor:row.secondaryColor,backgroundColor:row.backgroundColor,textColor:row.textColor}));applySettings();await putRecord('settings',row,{requireFirebase:true});renderSettings();showToast('Portada, Próximamente, Sale y configuración guardados correctamente.');
