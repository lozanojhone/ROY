...
function buildWhatsAppUrl(message){
 ...
 return 'https://api.whatsapp.com/send/?phone='+encodeURIComponent(number)+'&text='+encodeURIComponent(message)+'&type=phone_number&app_absent=0';
}
...
async function finishOrder(){
 ...
 const name=$('#checkoutName')?.value.trim()||'',dni=($('#checkoutDni')?.value||'').replace(/\D/g,''),phone=$('#checkoutPhone')?.value.trim()||'',delivery=$('#checkoutDelivery')?.value||'pickup',cfg=getShippingConfig(),address=delivery==='pickup'?(cfg.pickupAddress||state.settings.address||'Dirección pendiente'):($('#checkoutAddress')?.value.trim()||''),city=delivery==='pickup'?(cfg.pickupCity||'Cajamarca'):($('#checkoutCity')?.value.trim()||'');
 const items=cartDetails().map(x=>({productId:x.productId,name:x.product.name,sku:x.product.sku,color:x.color,size:x.size,qty:x.qty,price:x.product.price,subtotal:x.product.price*x.qty,image:x.product.image}));
 if(!items.length)return showToast('El carrito está vacío.');
 const c=calcCheckout();
 const order={id:'ROY-'+Date.now().toString().slice(-8),client:{name,dni,phone,email:state.profile.email||'',address,city},items,payment:state.payment,paymentStatus:'Pendiente',delivery:delivery==='pickup'?'Recojo en tienda':'Envío a domicilio',shipping:c.shipping,discount:c.discount,total:c.total,status:'Pendiente',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),coupon:state.coupon?.code||''};
 const msg=buildWhatsAppOrder(order),whatsappUrl=buildWhatsAppUrl(msg);
 finishingOrder=true;
 const button=document.querySelector('#checkoutModal .checkout-side .btn-primary');
 const originalButtonHtml=button?.innerHTML||'';
 if(button){button.disabled=true;button.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> REGISTRANDO PEDIDO...';}

 // Inicia el guardado inmediatamente y abre WhatsApp desde el mismo clic.
 // Así el navegador no muestra una pestaña about:blank ni bloquea la apertura.
 const savePromise=putRecord('orders',order,{requireFirebase:true});
 ...
 try{
  await Promise.race([
   savePromise,
   new Promise((_,reject)=>setTimeout(()=>reject(new Error('Tiempo de registro agotado')),8000))
  ]);
  if(!state.orders.some(x=>x.id===order.id))state.orders.unshift(order);
  saveLocal('orders',state.orders);
  state.profile={...state.profile,name,dni,phone,address:delivery==='delivery'?address:state.profile.address||'',city:delivery==='delivery'?city:state.profile.city||''};
  localStorage.setItem('roy_profile',JSON.stringify(state.profile));
  state.cart=[];persistCart();closeModal('checkoutModal');renderEverything();
  showToast('Pedido registrado correctamente.');
  if(!whatsappOpened){
   const go=confirm('El navegador bloqueó WhatsApp. ¿Deseas abrirlo ahora?');
   if(go)window.location.href=whatsappUrl;
  }
 }catch(error){
  console.error('No se pudo registrar el pedido:',error);
  showToast('WhatsApp se abrió, pero el pedido no se registró en el panel. Revisa las reglas de Firebase.');
 }finally{
  finishingOrder=false;
  if(button){button.disabled=false;button.innerHTML=originalButtonHtml||'<i class="fa-brands fa-whatsapp"></i> FINALIZAR COMPRA POR WHATSAPP';}
 }
}
...
function clearHeroFile(i){const input=$('#heroFile'+i);if(input){input.value='';delete input.dataset.preview}$('#heroImage'+i).value='';$('#heroPreview'+i).src='https://placehold.co/1600x800/111111/a8ff00?text=ROY';$('#heroStatus'+i).textContent='Imagen pendiente'}
async function resolveHeroSlidesForSave(){
 ...
 for(let i=0;i<3;i++){
  ...
  if(file){const ext=(file.name.split('.').pop()||'jpg').replace(/[^a-z0-9]/gi,'').toLowerCase();image=await uploadImageAsset(file,`roy/hero/banner_${i+1}_${Date.now()}.${ext}`)}
...
async function saveSettings(){
 ...
 try{
  ...
  saveLocal('settings',[row]);localStorage.setItem('roy_brand_cache',JSON.stringify({storeName:row.storeName,logo:row.logo,primaryColor:row.primaryColor,secondaryColor:row.secondaryColor,backgroundColor:row.backgroundColor,textColor:row.textColor}));applySettings();await putRecord('settings',row,{requireFirebase:true});renderSettings();showToast('Marca, colores, portada y configuración guardados correctamente.');
...
window.addEventListener('firebase-ready',()=>setSyncLabel(!!window._firebaseReady));
