...
'use strict';
const OFFICIAL_ROY_LOGO='assets/logo-roy-dorado-blanco.png';
...
const state={products:[],orders:[],categories:[],promotions:[],messages:[],reviews:[],users:[],inventory:[],media:[],cart:JSON.parse(localStorage.getItem('roy_cart')||localStorage.getItem(LEGACY_DB_PREFIX+'cart')||'[]'),favorites:JSON.parse(localStorage.getItem('roy_favorites')||localStorage.getItem(LEGACY_DB_PREFIX+'favorites')||'[]'),profile:JSON.parse(localStorage.getItem('roy_profile')||localStorage.getItem(LEGACY_DB_PREFIX+'profile')||'{}'),settings:{storeName:'ROY',whatsapp:'51926884544',email:'roycajamarca@gmail.com',hours:'Lunes a sábado: 8:00 AM a 8:00 PM',address:'Cajamarca, Perú · Cerca de la Plaza de Armas',facebook:'',instagram:'',tiktok:'',logo:OFFICIAL_ROY_LOGO,primaryColor:'#D4AF37',secondaryColor:'#ffffff',backgroundColor:'#080808',textColor:'#ffffff',brandVersion:30,heroSlides:[{image:'https://images.unsplash.com/photo-1523398002811-999ca8dec234?auto=format&fit=crop&w=1800&q=88',kicker:'Nueva colección 2026',title:'Viste tu propia actitud',highlight:'propia actitud',description:'Moda urbana, oversize y streetwear creada para quienes convierten cada prenda en identidad.',buttonText:'Explorar colección',buttonPage:'tienda'},{image:'https://images.unsplash.com/photo-1529139574466-a303027c1d8b?auto=format&fit=crop&w=1800&q=88',kicker:'Drop exclusivo',title:'Diseño que impone presencia',highlight:'impone presencia',description:'Siluetas modernas, detalles premium y el contraste oficial de ROY.',buttonText:'Ver productos',buttonPage:'tienda'},{image:'https://images.unsplash.com/photo-1506629082955-511b1aa562c8?auto=format&fit=crop&w=1800&q=88',kicker:'Streetwear ROY',title:'Haz que tu estilo hable',highlight:'tu estilo hable',description:'Descubre combinaciones creadas para destacar de día y de noche.',buttonText:'Ver lookbook',buttonPage:'galeria'}],heroInterval:5500,upcomingPromo:{image:'',eyebrow:'Próximo lanzamiento',title:'NEXT DROP',highlight:'DROP',description:'Nuevas siluetas, materiales y combinaciones están en desarrollo. Regístrate para recibir el anuncio antes que nadie.'},salePromo:{image:'',eyebrow:'Últimas unidades',title:'Urban Sale',highlight:'Sale',description:'Precios especiales por tiempo limitado. El descuento se aplica directamente.'},contactEyebrow:'Estamos donde nace la acción',contactTitle:'Encuéntranos',contactDescription:'Visita nuestra tienda o escríbenos. Respondemos consultas, cambios, pedidos y asesoría de talla.',contactLocationTitle:'Cajamarca',contactFormTitle:'Escríbenos',contactSubmitText:'Enviar consulta',contactMapQuery:'Cajamarca, Perú',contactMapUrl:'',freeShipping:200,policies:'Cambios coordinados dentro de los 7 días, conservando la prenda sin uso y con etiquetas.',terms:'Los pedidos se confirman al validar disponibilidad y pago.'},payment:'WhatsApp',coupon:null,orderStatusFilter:'Todos'};
...
const seedPromotions=[{id:'promo10',code:'ROY10',discount:10,description:'10% de descuento en tu primera compra',start:'2026-01-01',end:'2027-12-31',active:true}];
const seedUsers=[{id:'usr_admin',name:'Administrador ROY',username:'admin',role:'Administrador',permissions:'Todos los módulos',active:true}];
...
async function loadCollection(name,fallback=[]){
 ...
 if(window._firebaseReady&&window._db&&window._fb){
  try{
   ...
   if(name==='products'){saveLocal(name,remote);localStorage.setItem('roy_products_verified','1');return remote}
...
async function initData(){
 ...
 const verifiedProducts=localStorage.getItem('roy_products_verified')==='1'?loadLocal('products',[]):[];
 ...
 if(!state.settings.logo)state.settings.logo=OFFICIAL_ROY_LOGO;
 ...
 try{
  ...
  if(!state.settings.logo)state.settings.logo=OFFICIAL_ROY_LOGO;
  const renameBrand=value=>String(value||'').replace(/JHONZ/gi,'ROY');
  state.products=state.products.map(p=>({...p,brand:'ROY',name:renameBrand(p.name),description:renameBrand(p.description)}));
  state.media=state.media.map(m=>({...m,title:renameBrand(m.title),description:renameBrand(m.description)}));
  state.settings.email=String(state.settings.email||'roycajamarca@gmail.com').replace(/jhonz/gi,'roy');
...
function renderEverything(){fillCategorySelects();renderStore();renderFeatured();renderSale();renderCart();renderAccount();renderGallery();renderAllAdmin()}
...
function persistCart(){localStorage.setItem('roy_cart',JSON.stringify(state.cart));renderCartCount()}
...
function toggleFavorite(id){state.favorites=state.favorites.includes(id)?state.favorites.filter(x=>x!==id):[...state.favorites,id];localStorage.setItem('roy_favorites',JSON.stringify(state.favorites));renderStore();renderFeatured();renderSale();renderAccount()}
...
function applyDeliveryMode(){
 ...
 const cityLabel=$('#checkoutCityLabel'),addressLabel=$('#checkoutAddressLabel'),notice=$('#pickupNotice'),summary=$('#checkoutPickupSummary'),pickupMapLink=$('#checkoutPickupMapLink');
 if(delivery==='pickup'){
  ...
  if(city){city.value=cfg.pickupCity||'Cajamarca';city.readOnly=true;city.placeholder='';}
  ...
  if(summary)summary.textContent=(cfg.pickupAddress||state.settings.address||'Dirección pendiente')+' · '+(cfg.pickupCity||state.settings.contactLocationTitle||'Cajamarca');
  if(pickupMapLink){pickupMapLink.href=contactMapDestination(state.settings);pickupMapLink.style.display='inline-flex'}
  ...
  if(city){city.value=state.profile.city||'';city.readOnly=false;city.placeholder='Ejemplo: Lima, Cusco, Cajamarca';}
  ...
  if(pickupMapLink)pickupMapLink.style.display='none';
...
function makeQrCodes(){const base='https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=';$('#yapeQr').src=base+encodeURIComponent('YAPE ROY 926884544');$('#plinQr').src=base+encodeURIComponent('PLIN ROY 926884544')}
...
const PENDING_ORDER_KEY='roy_pending_orders';
...
async function finishOrder(){
 ...
 const orderId='ROY-'+Date.now().toString(36).toUpperCase()+'-'+Math.random().toString(36).slice(2,6).toUpperCase();
 const now=new Date().toISOString();
 const order={
  id:orderId,
  orderCode:orderId,
  source:'ROY Catalogo web',
  channel:'WhatsApp',
  currency:'PEN',
  client:{
   name,dni,phone,
 ...
 localStorage.setItem('roy_profile',JSON.stringify(state.profile));
...
function buildWhatsAppOrder(o){return `Hola ROY 👋 Quiero confirmar mi pedido ${o.id}\n\nCliente: ${o.client.name}\nDNI: ${o.client.dni||'No indicado'}\nWhatsApp: ${o.client.phone}\nEntrega: ${o.delivery}\nDirección: ${o.client.address||'Recojo en tienda'} ${o.client.city||''}\nPago: ${o.payment}\n\n${o.items.map(i=>`• ${i.name} | ${i.color} | Talla ${i.size} | x${i.qty} = ${money(i.subtotal)}`).join('\n')}\n\nEnvío: ${money(o.shipping)}\nDescuento: ${money(o.discount)}\nTOTAL: ${money(o.total)}\n\n${o.payment==='WhatsApp'?'Deseo coordinar el pago.':'Adjuntaré mi comprobante de '+o.payment+'.'}`}
function normalizePhone(v){let s=String(v||'').replace(/\D/g,'');if(s.length===9)s='51'+s;return s||'51926884544'}
function openWhatsApp(){window.open('https://wa.me/'+normalizePhone(state.settings.whatsapp)+'?text='+encodeURIComponent('Hola ROY, deseo información sobre sus prendas.'),'_blank')}
...
function openSupport(section='help'){
 const name=state.settings.storeName||'ROY';
 const data={
...
function contactHeadingMarkup(value){const raw=String(value||'').trim();if(!raw)return '';const words=raw.split(/\s+/);if(words.length>1){const last=words.pop();return esc(words.join(' '))+' <span>'+esc(last)+'</span>'}if(raw.length>4)return esc(raw.slice(0,-3))+'<span>'+esc(raw.slice(-3))+'</span>';return '<span>'+esc(raw)+'</span>'}
function normalizedContactMapUrl(value){const raw=String(value||'').trim();if(!raw)return '';try{const u=new URL(raw);const host=u.hostname.toLowerCase().replace(/^www\./,'');const allowed=host==='google.com'||host.endsWith('.google.com')||host==='maps.app.goo.gl'||host==='goo.gl';return allowed&&/^https?:$/.test(u.protocol)?u.href:''}catch{return ''}}
function contactMapEmbedSource(settings){const exact=normalizedContactMapUrl(settings?.contactMapUrl);if(exact){try{const u=new URL(exact);if(u.pathname.includes('/maps/embed')||u.searchParams.get('output')==='embed')return exact;const decoded=decodeURIComponent(exact);let m=decoded.match(/@(-?\d+(?:\.\d+)?),(-?\d+(?:\.\d+)?)/);if(!m)m=decoded.match(/[?&](?:q|query)=(-?\d+(?:\.\d+)?),(-?\d+(?:\.\d+)?)/);if(m)return 'https://www.google.com/maps?q='+encodeURIComponent(m[1]+','+m[2])+'&output=embed'}catch{}}const query=String(settings?.contactMapQuery||settings?.address||'Cajamarca, Perú').trim();return 'https://www.google.com/maps?q='+encodeURIComponent(query)+'&output=embed'}
function contactMapDestination(settings){const exact=normalizedContactMapUrl(settings?.contactMapUrl);if(exact)return exact;const query=String(settings?.contactMapQuery||settings?.address||'Cajamarca, Perú').trim();return 'https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(query)}
function applySettings(){
 const s=state.settings,name=(s.storeName||'ROY').trim()||'ROY';
 const primary=normalizeLuxuryPrimary(s.primaryColor),secondary=s.secondaryColor||'#ffffff',background=s.backgroundColor||'#080808',text=s.textColor||'#ffffff';
 ...
 $$('.logo-mark').forEach(mark=>{if(s.logo){mark.classList.add('has-image');mark.innerHTML='<img src="'+esc(s.logo)+'" alt="Logo '+esc(name)+'">'}else{mark.classList.remove('has-image');mark.textContent=(name.replace(/[^A-Za-z0-9]/g,'').slice(0,2)||'R').toUpperCase()}});
 ...
 const contactText={publicContactEyebrow:s.contactEyebrow||'Estamos donde nace la acción',publicContactTitle:s.contactTitle||'Encuéntranos',publicContactDescription:s.contactDescription||'Visita nuestra tienda o escríbenos. Respondemos consultas, cambios, pedidos y asesoría de talla.',publicContactLocationTitle:s.contactLocationTitle||'Cajamarca',publicContactFormTitle:s.contactFormTitle||'Escríbenos',publicContactSubmit:s.contactSubmitText||'Enviar consulta'};
 Object.entries(contactText).forEach(([id,value])=>{const el=$('#'+id);if(el)el.textContent=value});
 const contactTitle=$('#publicContactTitle'),contactFormTitle=$('#publicContactFormTitle');if(contactTitle)contactTitle.innerHTML=contactHeadingMarkup(s.contactTitle||'Encuéntranos');if(contactFormTitle)contactFormTitle.innerHTML=contactHeadingMarkup(s.contactFormTitle||'Escríbenos');
 const locationDestination=contactMapDestination(s),locationLabel=s.address||s.contactLocationTitle||'Ubicación ROY';
 const contactMap=$('#publicContactMap'),contactMapLink=$('#publicContactMapLink');if(contactMap)contactMap.src=contactMapEmbedSource(s);if(contactMapLink)contactMapLink.href=locationDestination;
 const catalogLocationLink=$('#catalogLocationLink'),catalogLocationText=$('#catalogLocationText'),footerLocationLink=$('#footerLocationLink'),footerLocationText=$('#footerLocationText'),footerCityText=$('#footerCityText'),checkoutPickupMapLink=$('#checkoutPickupMapLink');
 if(catalogLocationLink)catalogLocationLink.href=locationDestination;if(catalogLocationText)catalogLocationText.textContent=locationLabel;if(footerLocationLink)footerLocationLink.href=locationDestination;if(footerLocationText)footerLocationText.textContent=locationLabel;if(footerCityText)footerCityText.textContent=s.contactLocationTitle||'Cajamarca, Perú';if(checkoutPickupMapLink)checkoutPickupMapLink.href=locationDestination;
 renderHeroSlider();renderCampaignSections();
}
...
function mediaSection(item){const section=String(item?.section||'gallery');return ['gallery','upcoming','sale'].includes(section)?section:'gallery'}
function mediaSectionMeta(section){return section==='upcoming'?{label:'Próximamente',grid:'adminUpcomingGrid',search:'upcomingMediaSearch',type:'upcomingMediaType'}:section==='sale'?{label:'Sale',grid:'adminSaleGrid',search:'saleMediaSearch',type:'saleMediaType'}:{label:'Galería',grid:'adminMediaGrid',search:'mediaSearch',type:'mediaTypeFilter'}}
function mediaSectionPermission(section){return section==='upcoming'?'upcoming':section==='sale'?'sale':'gallery'}
function canManageMediaSection(section){const p=window._firebasePermissions||[];return p.includes('*')||p.includes(mediaSectionPermission(section))}
function requireMediaSectionPermission(section){if(canManageMediaSection(section))return true;showToast('Tu usuario no tiene permiso para administrar '+mediaSectionMeta(section).label+'.');return false}
function mediaAvailabilityMeta(value,section='upcoming'){
 const status=String(value||'').trim()||(section==='upcoming'?'coming-soon':'available');
 if(status==='coming-soon')return{value:'coming-soon',label:'Próximamente',button:'Se habilitará cuando esté disponible',enabled:false,cls:'coming'};
 if(status==='inquiry')return{value:'inquiry',label:'Consultar disponibilidad',button:'Consultar disponibilidad por WhatsApp',enabled:true,cls:'inquiry'};
 return{value:'available',label:'Disponible',button:'Consultar por WhatsApp',enabled:true,cls:'available'}
}
function campaignPriceMarkup(m){const price=Number(m?.price||0);return price>0?`<div class="campaign-product-price">${money(price)}</div>`:''}
function contactCampaignWhatsApp(id){
 const m=state.media.find(x=>x.id===id);if(!m)return;
 const section=mediaSection(m),status=mediaAvailabilityMeta(m.availability,section);if(!status.enabled)return showToast('Este producto todavía no está disponible.');
 const price=Number(m.price||0)>0?' · '+money(m.price):'';
 const context=section==='sale'?'la oferta':'el próximo lanzamiento';
 const message=`Hola ROY 👋 Quiero consultar por ${context}: ${m.title}${price}. ¿Me pueden dar más información y confirmar disponibilidad?`;
 window.open(buildWhatsAppUrl(message),'_blank','noopener,noreferrer')
}
function mediaClientCard(m){const section=mediaSection(m),body=m.type==='image'?`<img loading="lazy" src="${esc(m.url)}" alt="${esc(m.title)}">`:videoMarkup(m.url,m.title),status=mediaAvailabilityMeta(m.availability,section),commerce=section!=='gallery';return `<article class="campaign-media-card"><div class="campaign-media-preview">${body}</div><div class="campaign-media-body"><div class="campaign-card-top"><h3>${esc(m.title)}</h3>${commerce?`<span class="campaign-status ${status.cls}">${esc(status.label)}</span>`:''}</div>${commerce?campaignPriceMarkup(m):''}${m.description?`<p>${esc(m.description)}</p>`:''}${commerce?`<button class="btn ${status.enabled?'btn-primary':'btn-secondary'} campaign-wa-btn" ${status.enabled?`onclick="contactCampaignWhatsApp('${m.id}')"`:'disabled'}><i class="fa-brands fa-whatsapp"></i> ${esc(status.button)}</button>`:''}</div></article>`}
function renderCampaignMediaSections(){[['upcoming','upcomingCampaignGrid'],['sale','saleCampaignGrid']].forEach(([section,id])=>{const box=$('#'+id);if(!box)return;const list=state.media.filter(x=>x.active!==false&&mediaSection(x)===section);box.innerHTML=list.length?list.map(mediaClientCard).join(''):`<div class="campaign-empty">${section==='upcoming'?'Aún no hay próximos lanzamientos publicados.':'Aún no hay promociones visuales publicadas.'}</div>`})}
function renderGallery(){const gallery=state.media.filter(x=>mediaSection(x)==='gallery'),imgs=gallery.filter(x=>x.active!==false&&x.type==='image'),vids=gallery.filter(x=>x.active!==false&&x.type==='video');const fallback=[['City essentials','https://images.unsplash.com/photo-1529139574466-a303027c1d8b?auto=format&fit=crop&w=700&q=85'],['Bold energy','https://images.unsplash.com/photo-1539109136881-3be0616acf4b?auto=format&fit=crop&w=700&q=85'],['Street neutral','https://images.unsplash.com/photo-1506629082955-511b1aa562c8?auto=format&fit=crop&w=700&q=85']];$('#lookbookImages').innerHTML=(imgs.length?imgs:fallback.map((x,i)=>({id:'f'+i,title:x[0],url:x[1]}))).map(x=>`<figure><img src="${esc(x.url)}" alt="${esc(x.title)}"><figcaption>${esc(x.title)}</figcaption></figure>`).join('');$('#galleryVideos').innerHTML=vids.map(v=>`<article>${videoMarkup(v.url,v.title)}<div class="caption">${esc(v.title)}</div></article>`).join('');renderCampaignMediaSections()}
function mediaAdminCard(m){const section=mediaSection(m),status=mediaAvailabilityMeta(m.availability,section),commerce=section!=='gallery';return `<article class="media-card${commerce?' campaign-admin-manage-card':''}"><div class="media-preview">${m.type==='image'?`<img src="${esc(m.url)}" alt="${esc(m.title)}">`:videoMarkup(m.url,m.title)}</div><div class="media-body"><h4>${esc(m.title)}</h4>${commerce?`<div class="campaign-admin-commerce">${Number(m.price||0)>0?`<b>${money(m.price)}</b>`:'<span>Sin precio</span>'}<span class="campaign-status ${status.cls}">${esc(status.label)}</span></div>`:''}<p class="muted campaign-admin-description">${esc(m.description||'')}</p><div class="action-row campaign-admin-actions"><button class="btn btn-secondary btn-sm" onclick="editMedia('${m.id}')">Editar</button><button class="btn btn-secondary btn-sm" onclick="toggleMedia('${m.id}')">${m.active===false?'Publicar':'Ocultar'}</button><button class="btn btn-danger btn-sm" onclick="deleteMedia('${m.id}')">Eliminar</button></div></div></article>`}
function renderMediaAdminSection(section){const meta=mediaSectionMeta(section),box=$('#'+meta.grid);if(!box)return;box.classList.toggle('campaign-admin-manage-grid',section!=='gallery');const q=($('#'+meta.search)?.value||'').toLowerCase(),type=$('#'+meta.type)?.value||'';const list=state.media.filter(x=>mediaSection(x)===section&&(!q||[x.title,x.description,x.type].join(' ').toLowerCase().includes(q))&&(!type||x.type===type));box.innerHTML=list.length?list.map(mediaAdminCard).join(''):`<div class="empty" style="grid-column:1/-1">No hay contenido en ${meta.label}.</div>`}
function renderAdminMedia(){renderMediaAdminSection('gallery')}
function renderAdminSectionMedia(section){renderMediaAdminSection(section)}
function resetMediaUpload(){delete $('#mediaFile').dataset.data;delete $('#mediaFile').dataset.kind;$('#mediaFile').value='';$('#mediaFileName').textContent=''}
function prepareMediaModal(section,titleHtml){const meta=mediaSectionMeta(section);$('#mediaSection').value=section;$('#mediaModalEyebrow').textContent=meta.label;$('#mediaFormTitle').innerHTML=titleHtml;toggleMediaFields();openModal('mediaModal')}
function openMediaForm(section='gallery'){if(!requireMediaSectionPermission(section))return;['mediaId','mediaTitle','mediaUrl','mediaDescription','mediaPrice'].forEach(id=>$('#'+id).value='');$('#mediaType').value='image';$('#mediaAvailability').value=section==='upcoming'?'coming-soon':'available';$('#mediaActive').checked=true;resetMediaUpload();prepareMediaModal(section,'Nuevo <span>contenido</span>')}
function openSectionMediaForm(section){openMediaForm(section)}
function editMedia(id){const m=state.media.find(x=>x.id===id);if(!m)return;const section=mediaSection(m);if(!requireMediaSectionPermission(section))return;$('#mediaId').value=m.id;$('#mediaType').value=m.type;$('#mediaTitle').value=m.title;$('#mediaUrl').value=m.url;$('#mediaDescription').value=m.description||'';$('#mediaPrice').value=Number(m.price||0)>0?Number(m.price):'';$('#mediaAvailability').value=mediaAvailabilityMeta(m.availability,section).value;$('#mediaActive').checked=m.active!==false;resetMediaUpload();prepareMediaModal(section,'Editar <span>contenido</span>')}
function toggleMediaFields(){const video=$('#mediaType').value==='video',input=$('#mediaFile'),group=$('#mediaFileGroup'),section=$('#mediaSection').value||'gallery',commerce=section!=='gallery';group.classList.toggle('hidden',video);document.querySelectorAll('.media-commerce-field').forEach(el=>el.classList.toggle('hidden',!commerce));input.accept='image/*';$('#mediaFileLabel').textContent='O subir archivo de imagen';$('#mediaUploadHelp').textContent=video?'En el plan gratuito pega un enlace público; el video se reproducirá dentro de la tienda.':'La imagen se comprime y guarda directamente en Firestore.'}
...
async function uploadMediaFile(id){const input=$('#mediaFile');return input?.dataset.data||''}
async function saveMedia(){
 try{
  const id=$('#mediaId').value||uid('media'),type=$('#mediaType').value,title=$('#mediaTitle').value.trim(),section=$('#mediaSection').value||'gallery';
  if(!requireMediaSectionPermission(section))return;
  ...
  const priceRaw=Number($('#mediaPrice').value||0);if(priceRaw<0)throw new Error('El precio no puede ser negativo.');
  const old=state.media.find(x=>x.id===id)||{},commerce=section!=='gallery',m={...old,id,type,title,url,section,description:$('#mediaDescription').value.trim(),price:commerce?priceRaw:0,availability:commerce?mediaAvailabilityMeta($('#mediaAvailability').value,section).value:'',active:$('#mediaActive').checked,updatedAt:new Date().toISOString(),createdAt:old.createdAt||new Date().toISOString()};
  ...
  closeModal('mediaModal');renderGallery();renderAdminMedia();renderAdminSectionMedia('upcoming');renderAdminSectionMedia('sale');showToast(old.id?'Contenido actualizado correctamente.':'Contenido guardado correctamente.');
 }catch(error){console.error('No se pudo guardar contenido:',error);showToast(error.message||'No se pudo guardar el contenido.')}
}
async function toggleMedia(id){const m=state.media.find(x=>x.id===id);if(!m)return;const section=mediaSection(m);if(!requireMediaSectionPermission(section))return;m.active=m.active===false;await putRecord('media',m);renderGallery();renderAdminMedia();renderAdminSectionMedia('upcoming');renderAdminSectionMedia('sale')}
async function deleteMedia(id){const m=state.media.find(x=>x.id===id);if(!m)return;const section=mediaSection(m);if(!requireMediaSectionPermission(section))return;if(!confirm('¿Eliminar este contenido?'))return;await removeRecord('media',id);state.media=state.media.filter(x=>x.id!==id);saveLocal('media',state.media);renderGallery();renderAdminMedia();renderAdminSectionMedia('upcoming');renderAdminSectionMedia('sale')}
...
async function refreshFirebaseAdminSession(){
 ...
 try{
  const snap=await window._fb.getDoc(window._fb.doc(window._db,'roy_admins',window._auth.currentUser.uid));
...
function renderAdminView(name){const map={dashboard:renderDashboard,catalog:renderAdminProducts,gallery:renderAdminMedia,upcoming:()=>renderAdminSectionMedia('upcoming'),'sale-content':()=>renderAdminSectionMedia('sale'),inventory:renderInventory,orders:renderAdminOrders,clients:renderClients,sales:renderSales,promotions:renderPromotions,shipping:renderShipping,payments:renderPayments,messages:renderMessages,reviews:renderReviews,users:renderUsers,reports:()=>{},settings:renderSettings};map[name]?.()}
function renderAllAdmin(){renderDashboard();renderAdminProducts();renderAdminMedia();renderAdminSectionMedia('upcoming');renderAdminSectionMedia('sale');renderInventory();renderAdminOrders();renderClients();renderSales();renderPromotions();renderShipping();renderPayments();renderMessages();renderReviews();renderUsers();renderSettings()}
...
async function uploadProductFile(id){const input=$('#pImageFile'),file=input.files?.[0];if(!file||!window._firebaseReady||!window._storage||!window._fb)return input.dataset.data||'';try{const path=window._fb.ref(window._storage,`roy/products/${id}_${Date.now()}.jpg`);await window._fb.uploadBytes(path,file);return await window._fb.getDownloadURL(path)}catch(e){console.warn('Storage no disponible:',e.message);return input.dataset.data||''}}
async function uploadImageAsset(file,path){
 if(!file)return '';
 if(!window._firebaseReady||!window._storage||!window._fb)throw new Error('Firebase Storage no está disponible.');
 const storageRef=window._fb.ref(window._storage,path);
 await window._fb.uploadBytes(storageRef,file,{contentType:file.type||'image/jpeg'});
 return await window._fb.getDownloadURL(storageRef);
}
async function resolveProductGalleryUploads(id){
 ...
 for(let i=0;i<3;i++){
  ...
  if(file){
   ...
   result.push(await uploadImageAsset(file,`roy/products/${id}/gallery/${i}_${Date.now()}.${ext}`));
...
async function resolveProductColorUploads(id){
 ...
 for(const color of colors){
  ...
  for(let i=0;i<3;i++){
   ...
   if(file){
    const ext=(file.name.split('.').pop()||'jpg').replace(/[^a-z0-9]/gi,'').toLowerCase();
    values.push(await uploadImageAsset(file,`roy/products/${id}/colors/${slug(color)}/${i}_${Date.now()}.${ext}`));
   }else{
...
async function saveProduct(){
 ...
 const id=$('#productId').value.trim()||uid('prd'),name=$('#pName').value.trim(),sku=$('#pSku').value.trim()||'ROY-'+Date.now().toString().slice(-6),category=$('#pCategory').value.trim(),price=Number($('#pPrice').value),stock=Number($('#pStock').value);
 ...
 try{
  ...
  if(mainFile){
   ...
   image=await uploadImageAsset(mainFile,`roy/products/${id}/main_${Date.now()}.${ext}`);
  }
  ...
  const p={...old,id,name,sku,category,brand:$('#pBrand').value.trim()||'ROY',price,oldPrice:Number($('#pOldPrice').value||0),stock,tags:parseList($('#pTags').value),colors:colors.length?colors:['Único'],sizes:sizes.length?sizes:['Única'],description:$('#pDescription').value.trim(),image,gallery:gallery.filter(x=>x&&x!==image).slice(0,3),colorImages,createdAt:old.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString(),updatedBy:window._firebaseUser?.uid||''};
  ...
  if(code.includes('permission-denied'))message='Firebase rechazó la edición. Revisa que las reglas permitan update en roy_products.';
  else if(code.includes('unauthorized'))message='Storage rechazó la imagen. Revisa las reglas de Almacenamiento.';
...
function getShippingConfig(){
 const remote=state.shipping?.find?.(x=>x.id==='main')||state.shipping?.[0];
 let saved={};try{saved=JSON.parse(localStorage.getItem('roy_shipping_config')||'{}')}catch{}
 const base=remote||saved||{};
 return {...base,id:'main',local:0,national:0,freeShipping:true,pickupAddress:state.settings.address||base.pickupAddress||'Cajamarca, Perú · Cerca de la Plaza de Armas',pickupCity:state.settings.contactLocationTitle||base.pickupCity||'Cajamarca',pickupMapUrl:state.settings.contactMapUrl||base.pickupMapUrl||'',agency:base.agency||'Shalom / Olva Courier',pickup:base.pickup!==false,delivery:base.delivery!==false};
}
function renderShipping(){
 const c=getShippingConfig();
 if($('#shippingSettings'))$('#shippingSettings').innerHTML=`<div class="form-group full"><label>Dirección oficial de la tienda y punto de recojo</label><input class="field" id="shipPickupAddress" value="${esc(c.pickupAddress||'')}" placeholder="Avenida, calle, número y referencia"><small class="muted">Esta es la misma dirección usada en Contáctanos, Catálogo ROY, checkout y pie de página.</small></div><div class="form-group"><label>Ciudad / sede</label><input class="field" id="shipPickupCity" value="${esc(c.pickupCity||'Cajamarca')}" placeholder="Cajamarca"></div><div class="form-group full"><label>Enlace exacto de Google Maps</label><input class="field" id="shipPickupMapUrl" value="${esc(c.pickupMapUrl||'')}" placeholder="https://maps.app.goo.gl/..."><small class="muted">La ubicación exacta se vinculará automáticamente con toda la tienda y el Catálogo ROY.</small></div><div class="form-group"><label>Delivery local</label><input class="field" value="GRATIS" readonly></div><div class="form-group"><label>Envío nacional</label><input class="field" value="GRATIS" readonly></div><div class="form-group"><label>Empresas de transporte</label><input class="field" id="shipAgency" value="${esc(c.agency)}"></div><div class="switch"><span>Recojo en tienda</span><input type="checkbox" id="shipPickup" ${c.pickup?'checked':''}></div><div class="switch"><span>Delivery disponible</span><input type="checkbox" id="shipDelivery" ${c.delivery?'checked':''}></div>`;
 const list=state.orders.filter(o=>['En preparación','Enviado'].includes(o.status));if($('#shippingTracking'))$('#shippingTracking').innerHTML=list.length?list.map(o=>`<div class="order-card"><div class="order-top"><b>${esc(o.id)} · ${esc(o.client?.name)}</b><span class="status ${orderStatusClass(o.status)}">${esc(o.status)}</span></div><small class="muted">${esc(o.delivery)} · ${esc(o.client?.address||'Recojo')} · ${esc(o.client?.city||'')}</small></div>`).join(''):'<div class="empty">No hay pedidos en preparación o enviados.</div>';
}
async function saveShippingSettings(){
 const rawMap=$('#shipPickupMapUrl')?.value.trim()||'',mapUrl=normalizedContactMapUrl(rawMap);if(rawMap&&!mapUrl)return showToast('El enlace del mapa no es válido. Usa Compartir → Copiar vínculo en Google Maps.');
 const c={id:'main',pickupAddress:$('#shipPickupAddress').value.trim(),pickupCity:$('#shipPickupCity').value.trim(),pickupMapUrl:mapUrl,local:0,national:0,freeShipping:true,agency:$('#shipAgency').value.trim(),pickup:$('#shipPickup').checked,delivery:$('#shipDelivery').checked,updatedAt:new Date().toISOString()};if(!c.pickupAddress||!c.pickupCity)return showToast('Completa la dirección y ciudad del punto de recojo.');
 try{await putRecord('shipping',c);state.shipping=[c];state.settings={...state.settings,address:c.pickupAddress,contactLocationTitle:c.pickupCity,contactMapUrl:mapUrl,contactMapQuery:c.pickupAddress,storeLocation:{address:c.pickupAddress,city:c.pickupCity,mapUrl}};const settingsRow={id:'main',...state.settings,updatedAt:new Date().toISOString()};saveLocal('settings',[settingsRow]);await putRecord('settings',settingsRow);localStorage.setItem('roy_shipping_config',JSON.stringify(c));renderEverything();showToast('Ubicación vinculada en todo ROY y Catálogo ROY.');}catch(e){console.error(e);showToast('NO SE GUARDÓ la ubicación: '+(e.message||'Error de Firebase'));}
}
...
function normalizedHeroSlides(){
 const fallback=[
  {image:'https://images.unsplash.com/photo-1523398002811-999ca8dec234?auto=format&fit=crop&w=1800&q=88',kicker:'Nueva colección 2026',title:'Viste tu propia actitud',highlight:'propia actitud',description:'Moda urbana, oversize y streetwear creada para quienes convierten cada prenda en identidad.',buttonText:'Explorar colección',buttonPage:'tienda'},
  {image:'https://images.unsplash.com/photo-1529139574466-a303027c1d8b?auto=format&fit=crop&w=1800&q=88',kicker:'Drop exclusivo',title:'Diseño que impone presencia',highlight:'impone presencia',description:'Siluetas modernas, detalles premium y el contraste oficial de ROY.',buttonText:'Ver productos',buttonPage:'tienda'},
  {image:'https://images.unsplash.com/photo-1506629082955-511b1aa562c8?auto=format&fit=crop&w=1800&q=88',kicker:'Streetwear ROY',title:'Haz que tu estilo hable',highlight:'tu estilo hable',description:'Descubre combinaciones creadas para destacar de día y de noche.',buttonText:'Ver lookbook',buttonPage:'galeria'}
 ];
...
function heroTitleHtml(slide){
 const title=esc(slide.title||'ROY');const highlight=String(slide.highlight||'').trim();
...
function renderHeroSlider(){
 ...
 track.innerHTML=slides.map((slide,i)=>`<article class="hero-slide ${i===heroSlideIndex?'active':''}" data-hero-index="${i}"><div class="hero-slide-bg" style="background-image:url('${esc(slide.image).replaceAll("'",'%27')}')"></div><div class="wrap hero-slide-content"><div class="hero-slide-copy"><div class="hero-kicker">● ${esc(slide.kicker||'ROY')}</div><h1>${heroTitleHtml(slide)}</h1><p>${esc(slide.description||'')}</p><div class="hero-ctas"><button class="btn btn-primary" onclick="showPage('${esc(slide.buttonPage||'tienda')}')">${slide.buttonPage==='galeria'?'IR A GALERÍA':'IR A TIENDA'} →</button><button class="btn btn-secondary" onclick="showPage('${slide.buttonPage==='galeria'?'tienda':'galeria'}')">${slide.buttonPage==='galeria'?'IR A TIENDA':'IR A GALERÍA'}</button></div></div></div></article>`).join('');
...
async function saveSettings(){
 ...
 try{
  ...
  const rawContactMapUrl=$('#setContactMapUrl')?.value.trim()||'';const contactMapUrl=normalizedContactMapUrl(rawContactMapUrl);if(rawContactMapUrl&&!contactMapUrl)throw new Error('El enlace del mapa no es válido. Pega el vínculo de Google Maps obtenido desde Compartir → Copiar vínculo.');
  state.settings={...state.settings,storeName:$('#setStoreName').value.trim()||'ROY',logo:logoData||$('#setLogoUrl').value.trim()||state.settings.logo||OFFICIAL_ROY_LOGO,primaryColor:$('#setPrimaryColor')?.value||'#D4AF37',secondaryColor:$('#setSecondaryColor')?.value||'#ffffff',backgroundColor:$('#setBackgroundColor')?.value||'#080808',textColor:$('#setTextColor')?.value||'#ffffff',brandVersion:30,heroSlides,heroInterval:Math.max(3000,Math.min(15000,Number($('#setHeroInterval').value||6)*1000)),contactEyebrow:$('#setContactEyebrow')?.value.trim()||'Estamos donde nace la acción',contactTitle:$('#setContactTitle')?.value.trim()||'Encuéntranos',contactDescription:$('#setContactDescription')?.value.trim()||'Visita nuestra tienda o escríbenos. Respondemos consultas, cambios, pedidos y asesoría de talla.',contactLocationTitle:$('#setContactLocationTitle')?.value.trim()||'Cajamarca',contactFormTitle:$('#setContactFormTitle')?.value.trim()||'Escríbenos',contactSubmitText:$('#setContactSubmitText')?.value.trim()||'Enviar consulta',contactMapUrl,contactMapQuery:$('#setAddress')?.value.trim()||'Cajamarca, Perú',storeLocation:{address:$('#setAddress')?.value.trim()||'',city:$('#setContactLocationTitle')?.value.trim()||'Cajamarca',mapUrl:contactMapUrl},whatsapp:$('#setWhatsapp').value.trim(),email:$('#setEmail').value.trim(),hours:$('#setHours').value.trim(),address:$('#setAddress').value.trim(),facebook:$('#setFacebook').value.trim(),instagram:$('#setInstagram').value.trim(),tiktok:$('#setTiktok').value.trim(),freeShipping:Number($('#setFreeShipping').value||200),policies:$('#setPolicies').value.trim(),terms:$('#setTerms').value.trim()};
  const row={id:'main',...state.settings,updatedAt:new Date().toISOString()};
  try{const currentShipping=getShippingConfig();const linkedShipping={...currentShipping,pickupAddress:row.address,pickupCity:row.contactLocationTitle,pickupMapUrl:row.contactMapUrl||'',updatedAt:new Date().toISOString()};localStorage.setItem('roy_shipping_config',JSON.stringify(linkedShipping));if(state.shipping?.length)state.shipping=[linkedShipping]}catch{}
  if(settingsDocumentBytes(row)>900*1024)throw new Error('Las imágenes de Configuración superan el límite seguro. Vuelve a seleccionar una imagen para que sea optimizada.');
  saveLocal('settings',[row]);localStorage.setItem('roy_brand_cache',JSON.stringify({storeName:row.storeName,logo:row.logo,primaryColor:row.primaryColor,secondaryColor:row.secondaryColor,backgroundColor:row.backgroundColor,textColor:row.textColor}));applySettings();await putRecord('settings',row,{requireFirebase:true});renderSettings();showToast('Portada y configuración guardadas correctamente.');
...
function exportProductsCSV(){downloadBlob(rowsToCSV(state.products.map(p=>({SKU:p.sku,Producto:p.name,Categoria:p.category,Marca:p.brand,Precio:p.price,PrecioAnterior:p.oldPrice,Stock:p.stock,Colores:(p.colors||[]).join(', '),Tallas:(p.sizes||[]).join(', '),Etiquetas:(p.tags||[]).join(', ')}))),'productos_roy.xls','application/vnd.ms-excel')}
function exportInventoryCSV(){downloadBlob(rowsToCSV(state.products.map(p=>({SKU:p.sku,Producto:p.name,Categoria:p.category,Stock:p.stock,Estado:p.stock<=0?'Agotado':p.stock<=5?'Stock bajo':'Disponible'}))),'inventario_roy.xls','application/vnd.ms-excel')}
function exportOrdersCSV(){downloadBlob(rowsToCSV(state.orders.map(o=>({Pedido:o.id,Fecha:dateText(o.createdAt),Cliente:o.client?.name,DNI:o.client?.dni||'',WhatsApp:o.client?.phone,Productos:(o.items||[]).map(i=>i.name+' x'+i.qty).join(', '),Pago:o.payment,EstadoPago:o.paymentStatus,Entrega:o.delivery,Estado:o.status,Total:o.total}))),'pedidos_roy.xls','application/vnd.ms-excel')}
function exportSalesCSV(){downloadBlob(rowsToCSV(filteredSales().map(o=>({Fecha:dateText(o.createdAt),Pedido:o.id,Cliente:o.client?.name,Pago:o.payment,Estado:o.status,Total:o.total}))),'ventas_roy.xls','application/vnd.ms-excel')}
function exportClientsCSV(){downloadBlob(rowsToCSV(uniqueClients().map(c=>({Cliente:c.name,DNI:c.dni||'',WhatsApp:c.phone,Correo:c.email,Direccion:c.address,Pedidos:c.orders,Total:c.total,UltimaCompra:dateText(c.last)}))),'clientes_roy.xls','application/vnd.ms-excel')}
function exportSalesPDF(){const list=filteredSales(),{jsPDF}=window.jspdf,doc=new jsPDF({orientation:'landscape'});doc.setFillColor(10,10,10);doc.rect(0,0,297,25,'F');doc.setTextColor(255,77,0);doc.setFontSize(20);doc.text('ROY · REPORTE DE VENTAS',14,16);doc.setTextColor(30,30,30);doc.setFontSize(9);let y=36;doc.setFont('helvetica','bold');doc.text('FECHA',14,y);doc.text('PEDIDO',45,y);doc.text('CLIENTE',80,y);doc.text('PAGO',155,y);doc.text('ESTADO',195,y);doc.text('TOTAL',270,y,{align:'right'});y+=7;doc.setFont('helvetica','normal');list.forEach(o=>{if(y>195){doc.addPage('landscape');y=20}doc.text(dateText(o.createdAt),14,y);doc.text(o.id,45,y);doc.text(String(o.client?.name||'').slice(0,30),80,y);doc.text(o.payment||'',155,y);doc.text(o.status||'',195,y);doc.text(money(o.total),270,y,{align:'right'});y+=7});doc.setFont('helvetica','bold');doc.setTextColor(255,77,0);doc.text('TOTAL: '+money(list.reduce((s,o)=>s+Number(o.total||0),0)),270,y+6,{align:'right'});doc.save('reporte_ventas_roy.pdf')}
