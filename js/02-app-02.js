...
function applyDeliveryMode(){
 ...
 if(delivery==='pickup'){
  ...
  if(summary)summary.textContent=(cfg.pickupAddress||state.settings.address||'Dirección pendiente')+' · '+(cfg.pickupCity||'Cajamarca');
...
function applySettings(){
 ...
 const contactMap=$('#publicContactMap');if(contactMap){const query=String(s.contactMapQuery||s.address||'Cajamarca, Perú').trim();contactMap.src='https://www.google.com/maps?q='+encodeURIComponent(query)+'&output=embed'}
...
function getShippingConfig(){const remote=state.shipping?.find?.(x=>x.id==='main')||state.shipping?.[0];if(remote)return {...remote,local:0,national:0,freeShipping:true};const saved=JSON.parse(localStorage.getItem('roy_shipping_config')||'{"id":"main","pickup":true,"pickupAddress":"Cajamarca, Perú · Cerca de la Plaza de Armas","pickupCity":"Cajamarca","agency":"Shalom / Olva Courier","delivery":true}');return {...saved,local:0,national:0,freeShipping:true}}
